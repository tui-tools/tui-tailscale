...
`--demo` runs every screen against one sample tailnet, both ends of it: a fake node joined to `https://headscale.example.com`, with two peers — `exit-gateway`, which offers itself as an exit node, and `office-router`, which serves a subnet — and the fake control plane that serves them, where the node is registered under `user@example.com` and `office-router` advertises a second subnet nobody approved yet, so `r` has something to approve; a laptop waits to register, so `R` has one to register (behind the danger confirm an OIDC control plane gets), and a second login the identity provider's policy refused shows why `R` will not register it; the DNS section carries a split domain and an extra record; two join profiles pre-fill `j`, and a login server under `.internal` (`https://headscale.lab.internal`) walks the private-CA step; the demo's tui-cert keeps a local CA and a pair it issued, so `S`'s own-certificate step and the CA step list them, and the file picker browses a made-up tree. Its unit runs but is disabled, the way a fresh install is usually left, so the readiness line and the end of `S` and `O` show the enable. Every key works, every command is built and previewed for real, and each confirmed one is applied to the fake — nothing on the host is read or changed.

`--demo=node-only` is the same node on a machine without headscale, the way a laptop joined to somebody else's control plane looks. `--demo=partial-reset` is a host after a partial reset: tailscaled stopped and disabled, headscale installed with its configuration and state directory deleted, so `u` and `i` have something to fix.

## Screens

`tab` (or `1` to `6`) switches between them. `j` and `h` are actions here, so the selection moves with the arrow keys. The header is two rows at any width, the title and the facts; a row wider than the terminal is cut at the edge with a trailing `…` rather than wrapped, so the title row never scrolls off the top. The header names both backends with their versions — `tailscale 1.102.4` and `headscale 0.29.3`, or `headscale: not installed` — and the keys belong to the screen they are pressed on: the node's on the node and peers screens, the control plane's on the other four. On a machine that is only a node, with no headscale here and the node joined to a control plane elsewhere, the control-plane tabs are dimmed and their first line says so: `this machine is a node; the control plane is <server>`. They still offer `i`, for the day this machine should run a control plane of its own.

- **node** — the backend state (running, stopped, logged out, waiting for approval), the login server and whether it is self-hosted, the owner, the hostname and MagicDNS name, the tailnet addresses, and the settings: accept routes, advertised routes, the exit node in use, whether this node offers itself as one, accept DNS, the client version and its health warnings. A login waiting for a browser shows its URL here until it completes. When tailscale is not installed, or tailscaled is not running, or refuses this user, the screen says so and what to do; a stopped or disabled tailscaled is started with `u`.
...
**Without a key**, tailscale prints a login URL. tui-tailscale takes it from the command's output and shows it in a dialog and alone on the status line: open it in a browser, on any machine, and log in. With a self-hosted Headscale that is your identity provider's OIDC login. The node joins as soon as the login completes, and the URL stays on the node screen until then. While a login is pending the tool re-reads the node every few seconds on its own (for about three minutes; `r` re-reads by hand after that), because headscale takes up to half a minute after the browser confirms to finish the registration. When the login went through, the URL on the status line is replaced by `joined <tailnet> as <user>` and the dialog closes. A URL that disappears is not taken for an expired login at once: after the browser confirms, tailscaled still reads as logged out for a moment, then starting, then running, so the status line says it is waiting for the node and keeps re-reading. Only a node that still needs a login after about fifteen seconds is reported as expired (`j` starts a new login); a node stuck starting is reported as such after about two minutes. A later read that finds the node running, `r` included, replaces an "expired" line with who joined. Wherever it is shown, the URL sits on a line of its own, flush left, never wrapped and never inside a frame, so a terminal selection copies the URL and nothing else. On a terminal narrower than the URL the line is cut at the edge; `tui-tailscale --check | jq -r .loginUrl` prints it whole.
...
When the client is installed but tailscaled is stopped or disabled (after `systemctl disable --now tailscaled`, or a partial reset), the node screen says so and `u` starts it instead: the dialog previews `systemctl enable --now tailscaled` (after `systemctl unmask tailscaled` when the unit is masked), and the node is read again, waiting a moment for the daemon to answer, until the screen shows where it stands, usually `logged out · j joins a tailnet`. The hint bar leads with `u start tailscaled` until then, and the control plane's readiness line names `u` before `j` while this host is not a node yet.
...
| ports | the host firewall lets new connections reach the control port: the port in `listen_addr` when headscale binds a public address (a NAT or port forward in front of the host can give clients another one in `server_url`), the `server_url` port behind a reverse proxy | `443/tcp is closed in the host firewall`: `f` hands it to tui-firewall, prefilled |
...
The ports step reads the host firewall, and never changes it: `tui-firewall --check` when the family's firewall tool is installed (it already understands ufw, firewalld and nftables), otherwise `nft -j list ruleset` or `iptables -S`, all escalated. Jumps and gotos are followed into the chains they name, which is where ufw keeps its rules (on Ubuntu the input chain is a drop policy and a list of jumps, and the accept for a port sits two chains down). A rule this reader cannot judge (a match on an interface or a source) never decides the answer; a jump it cannot follow, because its chain is not in what was read, makes the answer unknown rather than letting the policy after it decide; and a firewall it cannot read is reported as unknown rather than open or closed. A closed `41641/udp` is not a missing step, since peers then relay through DERP, but the ready line says so; with the embedded DERP relay on, so is its STUN port (`3478/udp` by default). `f`, on any control-plane screen, hands the terminal to tui-firewall with the ports readiness found closed (see [Opening the closed ports (`f`)](#opening-the-closed-ports-f)); the screen comes back, and re-reads, when it exits. When tui-firewall is not installed, `f` offers to install it from the tui-tools repository (the same one headscale comes from, added first when it is not configured), previewed like `i`. `r`, and the return from `f`, look for every binary again, so a tool installed in another terminal meanwhile is picked up without a restart. `--check` reports the answer as `readiness.ports`: the source, the two ports and what the firewall does to each (`open`, `closed` or `unknown`), and `stunPort` and `stun` when the embedded DERP relay is on.

### Opening the closed ports (`f`)

`f` does not leave you in front of an empty rule list with the ports to retype. It starts tui-firewall with the ports readiness read as closed, the control port first, and one comment that says what they are for:

```sh
tui-firewall --open 443/tcp,41641/udp --comment 'tailnet control plane and node'
```

tui-firewall opens its add form filled in for the first port; Enter previews the rule and `y` applies it, then the form for the next port opens, one form and one confirm per port, and `esc` skips one. Nothing is applied without that confirm. The ports are the control port over TCP, `41641/udp` for the nodes' direct connections and, with the embedded DERP relay on, its STUN port (`3478/udp` by default); a port readiness could not judge is left out. The values are held to tui-firewall's own rules before it starts (a port from 1 to 65535, `tcp` or `udp`, no port twice, a one-line comment of at most 128 characters). When nothing is closed, `f` starts plain `tui-firewall`, on its rule list.

`--open` exists since tui-firewall 0.6.0. `f` asks `tui-firewall --version` first, and an older (or unknown) version is started plain; when it hands the terminal back, the status line names the ports to add with `a` there. When tui-firewall is not installed, `f` offers its install first, as above.
...
With OIDC configured, `R` is the one way around the identity provider's policy, so it is careful about which registrations it finishes. Opening a `/register/<id>` URL sends the browser to the provider, and the provider's answer comes back to headscale's `/oidc/callback`, where `allowed_groups`, `allowed_domains` and `allowed_users` are applied. A login they turn away (headscale logs `user msg: unauthorised group`, or domain, or user, and a 401 on the callback) stays in the registration cache like any other, so the nodes screen lists it on a line of its own as refused by the identity provider's policy, with the list that refused it, and `R` will not register it. A registration whose browser login went to the provider and has no answer yet is the browser's to finish, and `R` will not register that one either. Any other registration on an OIDC control plane goes through a danger confirm that says it registers the node without the identity provider's policy. The callback does not name its registration, so a refusal is attributed to the newest one whose browser login is still open; since `R` refuses both kinds, that only chooses the words. `--check` counts the refused ones as `refusedRegistrations`, which are part of `pendingRegistrations` too.
...
- then **the package**: `apt-get install -y headscale` (every `apt-get` step under `env DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a`, shown in the preview, so needrestart's prompt cannot hang the install), `dnf install -y headscale`, on Arch `pacman -Syu --needed --noconfirm tui-tools/headscale` (`-Syu` because Arch supports no partial upgrade: the machine is upgraded with it, and the dialog says so), and on Omarchy `pacman -S --needed --noconfirm tui-tools/headscale`, against the databases the repository setup or the last `omarchy update` synced, since Omarchy refuses a direct `-Syu`. The repository-qualified name picks the mirror over Arch's own package. These steps come from tui-kit's companion builders, the same per-distribution rule every tool of the family installs by.
...
When headscale is installed but `/etc/headscale/config.yaml` is gone (a partial reset that deleted `/etc/headscale`), the control-plane screens say that the configuration is missing, `S`, `O` and the DNS edits refuse (each is a diff of that file), and `i` previews a reinstall of the package instead, which puts the example configuration back: `apt-get install --reinstall -y -o Dpkg::Options::=--force-confmiss headscale` (dpkg restores a deleted configuration file only with `--force-confmiss`), `dnf reinstall -y headscale`, or `pacman -S` of `tui-tools/headscale` (`-Syu` on Arch). `S` is next after it. A missing `/var/lib/headscale` alone is harmless, since the unit recreates it at the next start, and the users screen's panel says so.

### Pre-auth keys (`n` on the preauth keys screen)

Pick the owning user by id and optionally add the words `reusable`, `ephemeral` and an expiration like `30m`, `24h` or `7d` (default `24h`). The previewed command is `headscale preauthkeys create --user <id> [--reusable] [--ephemeral] --expiration <dur>`. Headscale prints the key once, and tui-tailscale shows it once and never stores it: on a dialog of its own, with the key on a line of its own below the frame, flush left, never wrapped and never cut, like the login URL, so a terminal selection copies the key and nothing else. headscale 0.29 keys are 88 characters. On a terminal narrower than that, no part of the key is drawn, because a cut key is one headscale rejects: the dialog says how many columns it needs, and the key appears whole as soon as the window is wide enough. The one alternative is explicit: `w` writes the key once to `/run/tui-tailscale/preauth-<date>-<time>.key`, owned by root with mode 600, through the standard input of `install -D -m 600 /dev/stdin <file>`, previewed and confirmed; `/run` is a tmpfs, so the file is gone at the next boot at the latest, and `sudo cat` reads it. The key is never in the status line, in an argv, in a preview or in `--check`, and closing the dialog forgets it. The list keeps showing prefixes only, like headscale's own CLI. A key without `reusable` is spent by its first join: the list marks it `spent` in the USED column and dims it, and the line above the list says that `n` with `reusable` makes a key that joins several machines.

![A pre-auth key, shown once](docs/screenshots/tui-tailscale-preauth.png)
...
### Relays: headscale's embedded DERP

The last question of `S`, after `dns.base_domain`, is where the relays come from, with the current answer in the title:

| Choice | What is written |
| --- | --- |
| keep the relays as they are | nothing |
| embedded DERP on this host, Tailscale's public relays next to it | the embedded relay on; `derp.urls` keeps Tailscale's public map (added back if the file lists only maps of its own) |
| embedded DERP on this host only | the embedded relay on; Tailscale's public map is taken out of `derp.urls`, so there is no fallback relay when this host cannot be reached |
| Tailscale's public relays only (offered when the relay is on) | `derp.server.enabled: false`, and the public map back in `derp.urls` when nothing else is listed |

The embedded choices ask one more thing, `stun_listen_addr` (`0.0.0.0:3478`, or what the file has), and write the `derp.server` keys headscale needs, as the same minimal diff as the rest of `S`: `enabled`, `region_id`, `region_code` and `region_name` (the file's own values kept, headscale's defaults 999, `headscale` and `Headscale Embedded DERP` where there are none), `stun_listen_addr`, `automatically_add_embedded_derp_region: true`, and `private_key_path` when the file has none. `ipv4` and `ipv6` follow `server_url`: an address in `server_url` is written in as the relay's address, and the documentation addresses headscale's example file ships (`198.51.100.1`, `2001:db8::1`) are emptied, so nodes resolve the `server_url` name instead of dialling an address that is not this host. On headscale's example file with a name in `server_url`, that is three changed lines.

The relay answers on `server_url`'s own port, and nodes reach it over TLS only: with an `http://` `server_url` the dialog warns that nodes cannot use it (STUN still answers). Its STUN listener needs its UDP port open in the host firewall; after the restart the readiness line reads it, says when it is closed, and `f` hands it to tui-firewall with the other closed ports. `readiness.relays` in `--check` then says `embedded` or `embedded+tailscale-public`.

![The relay question at the end of S](docs/screenshots/tui-tailscale-relays.png)
...
tui-tailscale --demo=node-only      # a machine that is only a node, headscale elsewhere
tui-tailscale --demo=partial-reset  # tailscaled stopped, headscale's config deleted
