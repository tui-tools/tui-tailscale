...
func TestServerSettingsFlow(t *testing.T) {
	...
	if a.picker.Selected() != transportOptions[headscale.TransportReverseProxy] {
	...
	a = enter(t, a)
	if a.inputPurpose != inputBaseDomain || a.input.Model.Value() != "tailnet.example.com" {
	...
	if !strings.Contains(a.confirm.Command, headscale.HeadscaleConfigPath) {
	...
	if a.confirm.Command != "sudo -n systemctl enable headscale" {
	...
	state, err := a.hs.Load(t.Context())
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if got := state.ControlPlane.ServerURL; got != "https://vpn.example.org" {
		t.Errorf("server_url after the flow = %q", got)
	}
	// The rest of the demo's configuration survived the round trip.
	if !strings.Contains(state.ControlPlane.Raw, "metrics_listen_addr: 127.0.0.1:9090") {
...
func TestOIDCFlowNeverShowsTheSecret(t *testing.T) {
	...
	a = pick(t, a, headscale.ProviderGeneric.Label)
	if a.mode != modeInput {
	...
	if !strings.Contains(a.confirm.Command, headscale.OIDCClientSecretPath) {
	...
	if cp.OIDC.ClientSecretPath != headscale.OIDCClientSecretPath {
...
func TestConfigWriteIsRefusedWhenUnreadable(t *testing.T) {
	a := newCPApp(t)
	a.setScreen(screenUsers)
	a.hsState.ControlPlane = headscale.ControlPlane{
		ConfigPath: headscale.HeadscaleConfigPath,
...
func TestRestartStepFollowsTheUnitState(t *testing.T) {
	for _, tc := range []struct {
		...
		{"fresh install", "inactive", "disabled",
			[]string{"sudo -n systemctl enable --now headscale"}},
		{"already enabled", "active", "enabled",
			[]string{"sudo -n systemctl restart headscale"}},
		{"running but disabled", "active", "disabled",
			[]string{"sudo -n systemctl enable headscale", "sudo -n systemctl restart headscale"}},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.enabled == "disabled" && after.ControlPlane.ServiceEnabled != "enabled" {
				t.Errorf("the unit is still %q after the flow",
					after.ControlPlane.ServiceEnabled)
			}
		})
	}
}
...
func TestFixOwnershipFlow(t *testing.T) {
	...
	if a.confirm.Command != "sudo -n chown -R headscale:headscale /var/lib/headscale" {
	...
	if a.mode != modeConfirm || a.confirm.Command != "sudo -n systemctl enable headscale" {
	...
	a.hsState = state
	if !state.ControlPlane.Ownership.OK() {
		t.Errorf("ownership after the fix = %+v", state.ControlPlane.Ownership)
	}
...
func TestFixOwnershipChainsEveryFile(t *testing.T) {
	fake := headscale.NewFake()
	fake.SetStat(headscale.FileStat{Path: headscale.OIDCClientSecretPath,
		User: "root", Group: "root", Mode: 0o600})
	fake.SetService("active", "enabled")
	a := newApp(tailscale.NewFake(), fake, theme.New(), nil)
	a.width, a.height = 100, 30
	a.hsState, _ = fake.Load(t.Context())
	a.loading = false
	a.setScreen(screenUsers)
	...
	for i, want := range []string{
		"sudo -n chown headscale:headscale " + headscale.OIDCClientSecretPath,
		"sudo -n chown -R headscale:headscale /var/lib/headscale",
		"sudo -n systemctl restart headscale",
	} {
...
func TestFixOwnershipNeedsACheck(t *testing.T) {
	a := newCPApp(t)
	a.setScreen(screenUsers)
	a.hsState.ControlPlane.Ownership = headscale.Ownership{}
	model, _ := a.Update(key("F"))
	a = model.(*app)
...
func TestListScreensWithTheUnitStopped(t *testing.T) {
	...
	a.hsState = state
	for _, s := range []screen{screenUsers, screenNodes,
		screenKeys} {
		a.setScreen(s)
		view := a.View()
		...
		if strings.Contains(view, "could not read Headscale") {
			t.Errorf("screen %d still reports a read failure", s)
		}
	}
	a.setScreen(screenUsers)
...
func TestStepCountWithEnableAndRestart(t *testing.T) {
	a := newCPApp(t) // the demo unit is active and disabled
	a.setScreen(screenUsers)
	model, _ := a.Update(key("S"))
	a = model.(*app)
	a = enter(t, a)
