...
func (a *app) startOIDCSettings() tea.Cmd {
	...
	oidc := a.hsState.ControlPlane.OIDC
	a.cpDraft = controlPlaneDraft{
		onlyStart: oidc.OnlyStartIfAvailable,
		pkce:      oidc.PKCE,
	}
	options := make([]string, 0, len(headscale.OIDCProviders()))
	for _, p := range headscale.OIDCProviders() {
		options = append(options, p.Label)
	}
	current := headscale.ProviderForIssuer(oidc.Issuer)
...
func (a *app) tookOIDCProvider(choice string) tea.Cmd {
	provider := headscale.ProviderGeneric
	for _, p := range headscale.OIDCProviders() {
		if p.Label == choice {
			provider = p
		}
	...
	if provider.Issuer == "" {
		issuer := a.hsState.ControlPlane.OIDC.Issuer
		if headscale.ProviderForIssuer(issuer).ID != provider.ID {
			issuer = ""
		}
		a.openInput(inputOIDCIssuer, "Identity provider — issuer URL",
			...
			"The IdP's issuer URL — the one its discovery document is served under. "+
				"tui-tailscale reads "+headscale.OIDCDiscoveryPath+" from it before saving, from this "+
...
func (a *app) tookOIDCIssuer(value string) tea.Cmd {
	if problem := headscale.ServerURLProblem(value); problem != "" {
	...
	// groups claim.
	if a.cpDraft.provider.ID == "" || a.cpDraft.provider.ID == headscale.ProviderGeneric.ID {
		if preset := headscale.ProviderForIssuer(value); preset.ID != headscale.ProviderGeneric.ID {
	...
	if !a.cpDraft.provider.GroupsClaim && a.cpDraft.provider.Issuer != "" {
		serverURL := a.hsState.ControlPlane.ServerURL
		if problem := headscale.RedirectProblem(serverURL); problem != "" {
			a.cpDraft.provider = headscale.OIDCProvider{}
	...
	a.openInput(inputOIDCClientID, "Identity provider — client id",
		"headscale", a.hsState.ControlPlane.OIDC.ClientID,
...
func (a *app) redirectHelp() string {
	serverURL := a.hsState.ControlPlane.ServerURL
	uri := headscale.RedirectURI(serverURL)
	if uri == "" {
...
func (a *app) tookOIDCClientID(value string) tea.Cmd {
	if !headscale.ValidClientID(value) {
		a.setStatusf(ui.StatusError, "not a valid client id: %q", value)
		return nil
	}
	a.cpDraft.clientID = value

	help := "Typed masked, written to " + headscale.OIDCClientSecretPath + " (mode 600, owned " +
		"by " + serviceAccount(a.hsState.ControlPlane) + " — the account this host's " +
	...
	if a.hsState.ControlPlane.OIDC.ClientSecretSet {
...
func (a *app) tookOIDCSecret(value string) tea.Cmd {
	...
	help := "Only users whose email domain is on this list may log in. Empty means no " +
		"domain rule. " + headscale.AllowListsRule
...
func (a *app) tookOIDCDomains(value string) tea.Cmd {
	a.cpDraft.domains = headscale.SplitList(value)
	if !a.cpDraft.provider.GroupsClaim {
	...
	a.openInput(inputOIDCGroups, "Identity provider — allowed groups",
		"vpn-users", strings.Join(a.hsState.ControlPlane.OIDC.AllowedGroups, ", "),
...
func (a *app) askOIDCUsers() tea.Cmd {
	a.openInput(inputOIDCUsers, "Identity provider — allowed users",
		"ana@example.com", strings.Join(a.hsState.ControlPlane.OIDC.AllowedUsers, ", "),
		"An allow list of individual addresses. Empty means no per-user rule. It is not "+
			"an exception to the domains: "+headscale.AllowListsRule+" A user outside "+
...
func (a *app) tookOIDCUsers(value string) tea.Cmd {
	a.cpDraft.users = headscale.SplitList(value)
	if outside := headscale.UsersOutsideDomains(a.cpDraft.users, a.cpDraft.domains); len(outside) > 0 {
	...
	if a.cpDraft.provider.Issuer != "" {
		a.cpDraft.scope = headscale.SplitList(headscale.DefaultOIDCScope)
		a.openPicker(pickerOIDCOnlyStart, "only_start_if_oidc_is_available", a.cpDraft.onlyStart)
		return nil
	}
	scope := strings.Join(a.hsState.ControlPlane.OIDC.Scope, " ")
	if scope == "" {
		scope = headscale.DefaultOIDCScope
	}
	a.openInput(inputOIDCScope, "Identity provider — scope",
		headscale.DefaultOIDCScope, scope,
		"The scopes requested at login. \""+headscale.DefaultOIDCScope+"\" is what headscale "+
...
func (a *app) tookOIDCScope(value string) tea.Cmd {
	scope := headscale.SplitList(value)
	if len(scope) == 0 {
		a.setStatusf(ui.StatusError, "scope cannot be empty (try %q)", headscale.DefaultOIDCScope)
		return nil
	}
...
func (a *app) discoverIssuer() tea.Cmd {
	cmd, err := headscale.BuildDiscoverIssuer(a.cpDraft.issuer)
	if err != nil {
		a.setStatus(ui.StatusError, err.Error())
		a.cpDraft.forgetSecret()
		return nil
	}
	a.setStatusf(ui.StatusInfo, "checking %s…", headscale.DiscoveryURL(a.cpDraft.issuer))
	backend := a.hs
	return func() tea.Msg {
		...
		if !headscale.DiscoveryLooksValid(out) {
...
func (a *app) confirmOIDCChain(msg discoveredMsg) tea.Cmd {
	settings := headscale.OIDCSettings{
		...
		ClearInlineSecret: a.hsState.ControlPlane.OIDC.ClientSecretInline,
	}
	...
	discovery := "The issuer answered " + headscale.DiscoveryURL(a.cpDraft.issuer) + " correctly."
	if !msg.ok {
	...
	cmd := a.openConfirmWith(discovery+fmt.Sprintf("\n\nStep 1 of %d — write the client secret to ", 2+tail)+
		headscale.OIDCClientSecretPath+", mode 600, owned by "+serviceAccount(cp)+" — the "+
...
func (a *app) oidcIntro() string {
	...
	if provider.ID == "" {
		provider = headscale.ProviderForIssuer(a.cpDraft.issuer)
	}
	lines := []string{"Provider: " + strings.SplitN(provider.Label, " — ", 2)[0] + ". " +
		provider.Gate, a.redirectHelp(), headscale.AllowListsRule}
	draft := headscale.OIDCConfig{Issuer: a.cpDraft.issuer, AllowedDomains: a.cpDraft.domains,
		AllowedGroups: a.cpDraft.groups, AllowedUsers: a.cpDraft.users}
	for _, w := range headscale.OIDCWarnings(draft) {
...
func serviceAccount(cp headscale.ControlPlane) string {
	user, group := cp.ServiceUser, cp.ServiceGroup
	if user == "" {
		user = headscale.DefaultServiceUser
	}
	if group == "" {
		group = headscale.DefaultServiceUser
	}
	return user + ":" + group
}
...
func (a *app) startFixOwnership() tea.Cmd {
	...
	own := a.hsState.ControlPlane.Ownership
	switch {
	case !own.Checked:
	...
	cmds, err := headscale.BuildFixOwnership(own.Issues)
	if err != nil {
		a.setStatus(ui.StatusError, err.Error())
		return nil
	}
...
func (a *app) confirmFixStep(issues []headscale.OwnershipIssue, cmds []runner.Command, i int) tea.Cmd {
	total := len(cmds) + headscale.TailSteps(a.hsState.ControlPlane)
...
func ownershipSummary(issues []headscale.OwnershipIssue) string {
...
func (a *app) confirmConfigWrite(intro string, edits []headscale.ConfigEdit) tea.Cmd {
	cp := a.hsState.ControlPlane
	updated, changes, err := headscale.EditConfig(cp.Raw, edits)
	if err != nil {
		a.cpDraft.forgetSecret()
		a.setStatus(ui.StatusError, err.Error())
		return nil
	...
	write, err := headscale.BuildWriteHeadscaleConfig(updated)
	body := intro + "\n\n" + headscale.RenderConfigDiff(cp.ConfigPath, changes)
...
func (a *app) confirmRestartHeadscale() tea.Cmd {
	a.cpDraft.forgetSecret()
	cp := a.hsState.ControlPlane
	if !headscale.ServiceNeedsEnable(cp.ServiceEnabled) {
		return a.confirmPlainRestart("Last step")
	}
	if cp.ServiceState != "active" {
		enable, err := headscale.BuildEnableHeadscale(true)
	...
	enable, err := headscale.BuildEnableHeadscale(false)
...
func (a *app) confirmPlainRestart(step string) tea.Cmd {
	restart, err := headscale.BuildRestartHeadscale()
	return a.openConfirmWith(
...
func (a *app) serverURLWarning(url string) string {
	...
	if url != "" {
		...
		if problem := headscale.ServerURLProblem(url); problem != "" {
			warnings = append(warnings, "server_url is not valid: "+problem)
		}
	}
	if w := headscale.ServerURLWarning(url,
		a.hsState.ControlPlane.OIDC.Configured()); w != "" {
		warnings = append(warnings, w)
	}
	if w := headscale.BaseDomainConflict(url,
		a.hsState.ControlPlane.BaseDomain); w != "" {
		warnings = append(warnings, w)
	}
