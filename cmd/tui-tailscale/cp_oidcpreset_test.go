...
func googleGroupsConfig(t *testing.T, fake *headscale.Fake) {
	t.Helper()
	state, _ := fake.Load(t.Context())
	raw := state.ControlPlane.Raw
...
func TestGooglePreset(t *testing.T) {
	...
	if a.pickerPurpose != pickerOIDCProvider ||
		a.picker.Selected() != headscale.ProviderGoogle.Label {
	...
	if !strings.Contains(a.input.Help,
		"Register https://headscale.example.com/oidc/callback as the OAuth client's redirect URI") {
	...
	if !strings.Contains(a.input.Help, headscale.AllowListsRule) {
	...
	body := a.confirm.Body
	for _, want := range []string{"Provider: Google", "no groups claim", headscale.AllowListsRule,
		"Register https://headscale.example.com/oidc/callback", "WARNING: allowed_users bo@mail.example.net"} {
...
func TestGooglePresetRefusesAnHTTPRedirect(t *testing.T) {
	...
	model, _ := a.Update(key("O"))
	a = model.(*app)
	a = pick(t, a, headscale.ProviderGoogle.Label)
...
func TestCheckReportsOIDCReadiness(t *testing.T) {
	for _, probe := range []bool{false, true} {
		...
		if err := runCheckWith(context.Background(), tailscale.NewFake(), headscale.NewFake(), nil, &out,
